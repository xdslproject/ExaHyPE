...
from xdsl.dialects.builtin import (StringAttr, ModuleOp, IntegerAttr, IntegerType,
      ...
      Float16Type, Float32Type, Float64Type, IntAttr, FloatAttr, UnitAttr,
...
"""
This is a transformation pass which converts the exahype dialect to standard MLIR dialects. This
...
class GetAssignedVariables(Visitor):
  ...
  def traverse_assign(self, assign:exahype.Assign):
...
def translate_toplevel(ctx: SSAValueCtx, op: Operation, block) -> Operation:
    if isinstance(op, exahype.Function):
        block.add_op(translate_fun_def(ctx, op))
    if isinstance(op, exahype.Kernel):
        block.add_op(translate_kernel_def(ctx, op))

def translate_module(input_module: Module) -> ModuleOp:
    # create an empty global context
    global_ctx = SSAValueCtx()
    body = Region()
    block = Block()
    for top_level_entry in input_module.ops:
        translate_kernel(ctx, op)
        #for module in top_level_entry.children.blocks[0].ops:
        #    print(f"trans toplevel {module}")
        #    translate_toplevel(global_ctx, module, block)

    #print(f"first block {block.ops.first}")
    #assert (len(block.ops)) == 1 and isinstance(block.ops.first, func.FuncOp)
...
def translate_kernel(ctx: SSAValueCtx,
                      fn_def: exahype.Kernel) -> Operation:



def translate_fun_def(ctx: SSAValueCtx,
                      fn_def: exahype.Function) -> Operation:
    """
...
def try_translate_stmt(ctx: SSAValueCtx,
    ...
    """
    if isinstance(op, exahype.CallExpr):
        return translate_call_expr_stmt(ctx, op)
    if isinstance(op, exahype.Return):
        return translate_return(ctx, op)
    if isinstance(op, exahype.Assign):
        return translate_assign(ctx, op)
    if isinstance(op, exahype.Loop):
...
def translate_return(ctx: SSAValueCtx,
                     return_stmt: exahype.Return) -> List[Operation]:
    """
...
def translate_loop(ctx: SSAValueCtx,
                  loop_stmt: exahype.Loop) -> List[Operation]:
    """
    ...
    start_expr, start_ssa=translate_expr(ctx, loop_stmt.start.blocks[0].ops.first)
    end_expr, end_ssa=translate_expr(ctx, loop_stmt.stop.blocks[0].ops.first)
    ...
    step_op = arith.Constant.create(properties={"value": IntegerAttr.from_index_int_value(1)}, result_types=[IndexType()])
...
def translate_assign(ctx: SSAValueCtx,
                      assign: exahype.Assign) -> List[Operation]:
    """
...
def translate_call_expr_stmt(ctx: SSAValueCtx,
                             call_expr: exahype.CallExpr, is_expr=False) -> List[Operation]:
    """
...
def try_translate_expr(ctx: SSAValueCtx,
    ...
    Returns None otherwise.
    """
    if isinstance(op, exahype.Constant):
        op = translate_constant(op)
        return op
    if isinstance(op, exahype.Array):
        # Return an xdsl (?) or LLVM array here
        #shape = [IntegerAttr(d) for d in op.shape] 
        shape = [1,6,6]
        arr = VectorType(op.element_type, shape)
        return None #llvm.LLVMArrayType([IntAttr(7), op.element_type])

    if isinstance(op, exahype.BinaryOperation):
        op = translate_binary_expr(ctx, op)
        return op
    if isinstance(op, exahype.Var):
...
def translate_constant(op: exahype.Constant) -> Operation:
    """
...
def translate_string_into_global_and_get_element_ptr(string_val: str):
    ...
    #element_pointer=llvm.GEPOp(global_lookup.results[0], llvm.LLVMPointerType.typed(IntegerType(8)), [0,0])
    return [global_lookup], global_lookup.results[0] #, element_pointer], element_pointer.results[0]
...
class LowerExaHyPEToStandard(ModulePass):

  name = 'exahype-to-standard'

  def apply(self, ctx: MLContext, input_module: ModuleOp):
    res_module = translate_module(input_module)
